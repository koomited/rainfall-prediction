...
class DataTransformation:
    ...
    def prepare_dataframe_for_lstm(self, df, n_steps=7):
        df = dc(df)
    ...
    def train_test_spliting(self):
        ...
        train_set.loc[:, features_no_time_clog + target] = train_set[
            features_no_time_clog + target
        ].interpolate(method="spline", order=3)
        test_set.loc[:, features_no_time_clog + target] = test_set[
            features_no_time_clog + target
        ].interpolate(method="spline", order=3)
        ...
        train_set = self.prepare_dataframe_for_lstm(
            df=train_set, n_steps=self.config.look_back
        )
        test_set = self.prepare_dataframe_for_lstm(
            df=test_set, n_steps=self.config.look_back
        )

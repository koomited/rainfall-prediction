...
class ModelTrainer:
    ...
    class LSTM(nn.Module):
        ...
        def forward(self, x):
            bacth_size = x.size(0)
            h0 = torch.zeros(self.num_stacked_layers, bacth_size, self.hidden_size).to(
                self.device
            )
            c0 = torch.zeros(self.num_stacked_layers, bacth_size, self.hidden_size).to(
                self.device
            )
    ...
    def train(self):
        ...
        classification_target_column = list(
            self.config.target_column_classification.keys()
        )[0]
        ...
        joblib.dump(
            lr,
            os.path.join(self.config.root_dir, self.config.classification_model_name),
        )
        ...
        factor, patience = float(self.config.lstm_model_params.scheduler_factor), int(
            self.config.lstm_model_params.scheduler_patience
        )
        ...
        torch.save(
            trained_model,
            os.path.join(self.config.root_dir, self.config.lstm_model_name),
        )

    ...
    def __init__(self, run_id):
        ...
        self.classifier = joblib.load(
            mlflow.artifacts.download_artifacts(
                f"{self.S3_BASE_URL}classifier/classification.joblib"
            )
        )
        self.scaler = joblib.load(
            mlflow.artifacts.download_artifacts(
                f"{self.S3_BASE_URL}scaler/scaler.joblib"
            )
        )
        self.regressor = torch.load(
            mlflow.artifacts.download_artifacts(
                f"{self.S3_BASE_URL}regressor/lstm.pth"
            ),
            weights_only=False,
        )
